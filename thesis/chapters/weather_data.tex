...
\caption[Structure of the \eacs{XML} document returned by the \eacs{API} of \emph{yr.no}]{Structure of the \eacs{XML} document returned by the \eacs{API} of \emph{yr.no}; attributes are omitted for better readability.}
...
\caption[A \texttt{<time>} element returned by the \eacs{API} of \emph{yr.no} (1)]{A typical \texttt{<time>} element returned by the \eacs{API} of \emph{yr.no}.}
...
\begin{itemize}
  ...
  \item Some \texttt{<location>} elements have the child elements \texttt{precipitation} and \texttt{symbol}; the values of the attributes \texttt{from} and \texttt{to} of the enclosing \texttt{<time>} element differ by three to six hours; see Listing~\ref{listing:yrno_time_element2} for an example of such an element.
\end{itemize}
...
\caption[A \texttt{<time>} element returned by the \eacs{API} of \emph{yr.no} (2)]{A \texttt{<time>} element returned by the \eacs{API} of \emph{yr.no} describing a period of time.}
