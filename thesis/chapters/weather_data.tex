...
\emph{KNX} sensors are available for
\begin{itemize} % TODO sort alphabetically
  ...
  \item temperature (\emph{thermometer}),
  ...
  \item atmospheric pressure (\emph{barometer}), and
...
Other sensors that are available under the \emph{KNX} standard include sensors for water, fluid level, smoke or $CO_2$ concentration. However, these are not relevant in the context of \thinkhomeweather as they do not provide weather data.
...
\begin{tabular}{|p{.2\textwidth}|p{.3\textwidth}|p{0.35\textwidth}|}
  ...
  \hline
  \textbf{\acs{METAR}} & Format standardised by \eacs{ICAO}; not human-readable. & Via HTML from \eacs{NOAA}'s web page (\emph{National Oceanic and Atmospheric Administration}). Various other data sources are available. \\ % TODO footnotes
  \hline
  \textbf{\acs{NWS}} & \eacs{XML} & \eacs{SOAP}\cite{SOAP}, \eacs{REST}\cite{REST} \\ % TODO references
  \hline
...
for the city of Vienna, Austria. An \eacs{HTTP} \emph{GET} request to this \emph{URL} returns an \eacs{XML} document conforming to the \eacs{XML} Schema definition that can be found
