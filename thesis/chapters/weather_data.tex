...
\item[3] Weather reports in \eacs{METAR} format are available in \eacs{HTML} format from \eacs{NOAA}'s web page\tnote{2} (\emph{National Oceanic and Atmospheric Administration}). Various other data sources are available.
\end{tablenotes}
\end{threeparttable}
...
\eacs{METAR} (\emph{Meteorological Aerodrome Report})~\cite{metar} is a format for reporting weather data~\cite{metar} that is standardised by the \emph{International Civil Aviation Organization} (\eacs{ICAO})~\cite{ICAO}. It is primarily used in aviation to provide weather data to pilots of aircraft. There are a few parsers available, e.g.\ \emph{PyMETAR}~\cite{pymetar} for Python or an implementation for Java~\cite{metar-java}.
...
\end{tabular}
\begin{tablenotes}
\item[1] \eacs{METAR} is standardised by \eacs{ICAO}~\cite{ICAO}. It was introduced in 1968 and has been modified a number of times since. However, most elements remained the same since introduction and can be expected to remain unchanged in the long run.
\end{tablenotes}
\end{threeparttable}
...
As seen in Table~\ref{table:weather_data6}, \eacs{METAR} only provides current weather data; some services provide forecasts for several days, but issue only one forecast per day (\emph{Yahoo! Weather}, \emph{Google Weather Feed}, \emph{Weather Underground}, and \emph{World Weather Online}). All weather services provide frequent updates (at least every few hours).
