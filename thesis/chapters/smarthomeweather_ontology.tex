% TODO their caption is -> their captions are
...
\begin{figure}
  ...
  \caption[An instance of \Egls{temperature} without units]{An instance of \Egls{temperature} together with the property \egls{has temperature value} representing a temperature of \SI{17.2}{\celsius} (without using a unit ontology).}
  \label{fig:muo1}
\end{figure}
...
\Egls{temperature} is defined to be a \Egls{weather phenomenon} which has a property \egls{has temperature value} specifying a temperature value (a numeric value of the type \texttt{xsd:float}) and its unit (degrees Celsius); all other sub-concepts of \Egls{weather phenomenon} are defined in a similar way:
...
\Egls{room temperature} is defined analogously; in addition to \Egls{temperature}, restrictions on the temperature values are added (at least \SI{20}{\celsius} and at most \SI{25}{\celsius}):
...
All other sub-concepts of \Egls{temperature} (\Egls{frost}, \Egls{cold}, \Egls{below room temperature}, \Egls{above room temperature}, and \Egls{heat}) are defined in the same way.
...
Depending on the instance of \Egls{interval} the property \egls{has start time} refers to, a \Egls{weather report} is inferred to be an instance of \Egls{current weather report}, \Egls{forecast weather report}, \Egls{short range weather report}, \emph{forecast 1 hour weather report} etc. For instance, \Egls{current weather report} is defined as follows:
