...
\textbf{Implementation language}: The ontology is implemented in \emph{\acs{OWL}~2}~\cite{OWL} using \protege~\cite{protege} and the \emph{Pellet reasoner}~\cite{pellet}.
...
\begin{itemize}
  ...
  \item \Egls{sun position} (Figure~\ref{fig:tree_sun_position}): There are sub-concepts that are defined depending on the sun's direction, while others are defined depending on the sun's elevation above horizon:
    \begin{itemize}
      \item Those depending on the direction are \Egls{sun from north} (direction of at least \SI{0}{\degree} and at most \SI{45}{\degree} or of more than \SI{315}{\degree} and less than \SI{360}{\degree}), \Egls{sun from east} (direction of more than \SI{45}{\degree} and at most \SI{135}{\degree}), \Egls{sun from south} (direction of more than \SI{135}{\degree} and at most \SI{225}{\degree}), and \Egls{sun from west} (direction of more than \SI{225}{\degree} and at most \SI{315}{\degree}).
...
\begin{itemize}
  ...
  \item \Egls{medium range weather report}: \emph{Forecast 6 hours weather report} and \emph{Forecast 9 hours weather report} for weather reports describing the weather in 6 and 9 hours, respectively.
  \item \Egls{long range weather report}: \emph{Forecast 12 hours weather report}, \emph{Forecast 15 hours weather report}, \emph{Forecast 18 hours weather report}, \emph{Forecast 21 hours weather report}, and \emph{Forecast 24 hours weather report} for weather reports describing the weather in 12, 15, 18, 21, and 24 hours, respectively.
\end{itemize}
...
The tables can be found in the appendix in Table~\ref{table:concept_dict1} (\Egls{weather condition}, \Egls{weather state}, and \Egls{weather source}) and Table~\ref{table:concept_dict2} (\Egls{weather phenomenon} and \Egls{weather report}). In these tables, sub-concepts having the same instances, class attributes, instance attributes, and relations as their super-concepts are omitted. Furthermore, any columns that are not filled with any content in any row are omitted.
...
\begin{itemize}
  ...
  \item The \emph{Basic Geo (\acs{WGS84} lat/long) Vocabulary}~\cite{wgs84_vocabulary} is used for specifying the location a \Egls{weather report} is valid for.
...
Section~\ref{sec:ontology_imports} describes the reuse of the aforementioned ontologies within the \smarthomeweather ontology in detail.
...
Figure~\ref{fig:muo1} shows how an instance of \Egls{temperature} would be implemented if no ontology for units of measurements would be used. With the introduction of the \emph{Measurement Units Ontology} (\muo), the data property and the literal are removed; an object property that is a sub-property of \emph{Quality value} (which is defined by \muo) takes the place of the data property. It links to a blank node which in turn has two properties: \emph{measured in} and \emph{numerical value}. The property \emph{measured in} is an object property that refers to the unit being used which is represented by an instance of \muo's concept \emph{Unit of measurement}. Using the data property \emph{numerical value}, the literal is connected to the blank node. The resulting pattern is seen in Figure~\ref{fig:muo2}.
...
Based on these reasoning capabilities in \smarthomeweather, it is possible to provide answers to some of the competency questions (cf.\ Section~\ref{sec:ontology_evaluation}). To answer all of the questions, \eacs{SWRL} rules and \eacs{SPARQL} queries may be implemented (see Section~\ref{sec:queries_rules}).
...
\subsection{Weather states satisfying certain conditions}
...
Restrictions about time and weather phenomena can be combined, e.g.\ to find all instances of \Egls{weather state} that represent a temperature of below \SI{0}{\celsius} during the next three hours:
...
\subsection{Rising and falling values of weather phenomena over time}
...
With these insights, it is possible to write the following query that returns all pairs of \Egls{weather state}s that indicate an increasing temperature value over time, i.e.\ the temperature value of \texttt{?s2} is higher than the temperature value of \texttt{?s1} while \texttt{?s2} describes a later weather state than \texttt{?s1}:
...
The following rule defines the semantics of a new property \emph{increasing temperature} which relates pairs of \Egls{weather state}s to each other in a way that indicates an increasing temperature value over time:
