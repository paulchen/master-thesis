...
\begin{itemize}
  \item \textbf{What is the current weather situation?}\\
    The current weather situation is represented by an instance of \Egls{current weather report}, an instance of \Egls{weather state} connected to it via the property \Egls{has weather state}, the instances of \Egls{Weather condition} linked to the \Egls{weather state} using the property \egls{has condition} and all instances of \Egls{weather phenomenon} associated with the \Egls{weather state} via the property \egls{has weather phenomenon}. These instances together with the values of their instance attributes provide information about the current weather state.
    
  \item \textbf{What will the weather situation be in one hour, in two hours, …, in 24 hours?}\\
    The weather situation in one hour is represented by an instance of \emph{Forecast 1 hour weather report} together with associated instances of \Egls{weather state}, \Egls{weather condition} and \Egls{weather phenomenon}. These instances provide information about the weather situation in one hour in the same way as information is provided about the current weather situation.\\
    The weather situations in 2, 3, 6, 9, 12, 15, 18, 21, and 24 hours are provided in the same way.
    
  \item \textbf{What is the current temperature, humidity, wind speed, …?}\\
    The current weather situation is represented by an instance of \Egls{current weather report}. Using the property \egls{has weather state}, an instance of \Egls{weather state} is connected. This \Egls{weather state} has an arbitrary number of instances of \Egls{weather phenomenon} linked to it via the property \egls{has weather phenomenon}. These instances of \Egls{weather phenomenon} provide current measurement values for all weather elements implemented in the ontology.\\
    Furthermore, instances of \Egls{weather condition} are linked to the \Egls{current weather report} using the property \egls{has condition}.
  ...
  \item \textbf{What will be the minimum temperature, humidity, … over the next 24 hours? What about maximum values?}\\ % TODO reference open world assumption
    Due to the \emph{Open World Assumption} (\emph{OWL}), it is not possible for an \emph{OWL} reasoner to determine minimum or maximum values of a set of values where values can be added. However, any program that accesses the ontology can find minimum and maximum values.
    
  \item \textbf{Will the weather change? Will the temperature, humidity, … rise or fall?}\\ % TODO HM
  ...
  \item \textbf{Does it rain? Will it rain in the next hours? Will it rain today?}\\
    If precipitation data is available, every instance of \Egls{weather state} has a property \egls{has weather phenomenon} that links an instance of \Egls{precipitation} to it. The attributes of that instance provide values for chance and intensity of rain. There are several sub-concepts of \Egls{precipitation} (\Egls{no rain}, \Egls{light rain}, \Egls{medium rain}, \Egls{heavy rain}, \Egls{extremely heavy rain} and \Egls{tropical storm rain}).\\
    Furthermore, there are sub-concepts of \Egls{weather state} regarding precipitation which are \Egls{no rain weather}, \Egls{rainy weather} and \Egls{very rainy weather}. These concepts provide general knowledge about precipitation for the respective \Egls{weather state}.\\
    Although the \thinkhomeweather ontology cannot tell whether it will rain in the next 24 hours, it can tell if it will be raining at a certain time in the next 24 hours. Any program using the ontology can use this information and draw conclusions based on it.
