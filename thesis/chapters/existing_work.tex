...
\begin{itemize}
  \item \textbf{\eacs{DOAP}}: \eacs{DOAP}~\cite{DOAP} (\emph{Description of a Project}) is an vocabulary for describing software projects.
  ...
  \item \textbf{\eacs{FOAF}}: \eacs{FOAF}~\cite{FOAFWeb,FOAF} is an ontology for describing social networks. It models persons, the relations between them, their activities and their relations to other objects.
  \item \textbf{\eacs{SIOC}}: The \eacs{SIOC}~\cite{SIOC} is a technology built around an ontology for encoding information from Internet discussion methods (message boards, blogs, mailing lists etc.).
