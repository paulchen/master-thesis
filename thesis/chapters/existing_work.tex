...
\begin{itemize}
  \item \emph{Earth Realm}
  \item \emph{Non-Living Element} 
  \item \emph{Living Element} 
  \item \emph{Physical Properties} 
  \item \emph{Units} 
  \item \emph{Numerical Entity} 
  \item \emph{Temporal Entity} 
  \item \emph{Spatial Entity} 
  \item \emph{Phenomena}
\end{itemize}

% TODO figure
Now, \eacs{SWEET} is now implemented in \eacs{OWL}. The current version, \eacs{SWEET} 2.3 which was last updated in 2012 consists of nine top-level concepts (see figure~\ref{fig:sweet_overview}).
...
\begin{itemize}
  ...
  \item … % TODO doesn't cover forecasts, 
  
  \item … % TODO doesn't cover many of the required weather elements
\end{itemize}
% TODO
...
\begin{itemize}
  \item \textbf{\eacs{SWEET}}: Besides concepts, attributes and individuals for atmospherical phenomena, \eacs{SWEET} also comes with support for literals more precisely specified by units~\cite{SWEET1}. However, as mentioned above, \eacs{SWEET} is an ontology that is inappropriate for use in \thinkhomeweather.
