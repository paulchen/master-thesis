...
This section covers \emph{Semantic Sensor Web}~\ref{subsec:onto1}, the \emph{\eacs{SSN} ontology}~\ref{subsec:onto2}, \emph{SemSOS}~\ref{subsec:onto3}, \emph{NextGen}~\ref{subsec:onto4}, and \emph{\eacs{SWEET}}~\ref{subsec:onto5}. There are several other approaches such as the \emph{SENSEI project}~\cite{sensei} which are not covered here.
...
Applications of \eacs{SSW} include work on situation awareness based on the metadata specified by \eacs{SSW}~\ref{ssw_example1} and an architecture for a distributed semantic sensor web~\ref{ssw_example2}.
...
\subsection{SSN Ontology}
\label{subsec:onto2}

Another approach towards semantically enriched sensor network based on \eacs{SWE} is an \emph{\eacs{OWL} 2} ontology created by the \emph{\eacs{W3C} Semantic Sensor Network Incubator group} (\eacs{SSN-XG})~\cite{SSN-XG} which is referred to as the \emph{\eacs{SSN} Ontology}~\cite{ssn_ontology}. 
