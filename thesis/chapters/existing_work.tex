...
As section~\ref{sec:weather_ontologies} discusses, unfortunately no existing ontology covers the domain of weather data in a way suitable for using it as a starting point for \smarthomeweather. \emph{Semantic Sensor Web} (section~\ref{subsec:onto1}) …; the \emph{\eacs{SSN} ontology} (section~\ref{subsec:onto2}) …; \emph{SemSOS} (section~\ref{subsec:onto3}) …; \emph{NextGen} (section~\ref{subsec:onto4}) …; and \emph{\eacs{SWEET}} (section~\ref{subsec:onto5}) ….

Thus, a completely new ontology is created (see chapter~\ref{ch:smarthomeweather_ontology}). From the insights gained above, the following aspects are considered for the development of \smarthomeweather:
