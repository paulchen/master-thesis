...
% TODO mention that everything will be done in Java
...
% TODO pro/cons table with all weather services?
...
\begin{itemize}
  ...
  \item \textbf{Coverage area:} Worldwide coverage. % TODO mention http://www.aviationweather.gov/static/adds/metars/stations.txt
...
\begin{table}
  ...
  \hline
  \textbf{Weather service} & \textbf{Advantage(s)} & \textbf{Disadvantage(s)} \\ % TODO replace (s) by s or omit completely?
  \hline \hline
  DWD & & \\
  \hline
...
Furthermore, an overall weather condition is added to the ontology. This weather condition is described by the following states: \emph{Cloudy}, \emph{fog}, \emph{light clouds}, \emph{partly cloudy}, \emph{rain}, \emph{sleet}, \emph{sun}, \emph{thunder}. At most three of them may be used together to describe the overall weather condition. These states are taken from the list of symbols used for describing the weather condition in the API of \emph{yr.no}.

% TODO table that assigns weather elements to competence questions
...
To ensure the ontology can be used in the desired manner, it is necessary that the competence questions from section \ref{sec:weather_information} can be answered by the ontology using the provided input data. See table \ref{table:competence_questions} for details about which competence questions can be answered by using which weather element(s). After developing the ontology, section \ref{sec:ontology_evaluation} will discuss whether this ontology can answer the competence questions.

\begin{table}
\begin{tabular}[h]{|l|l|} % TODO
\hline
\textbf{Competence question} & \textbf{Weather element(s)} \\ % TODO remove (s) or replace it by s?
\hline \hline
bla & \\
\hline
bla & \\
...
\hline
\end{tabular}
\label{table:competence_questions}
\caption{Assignment of competence questions to weather element(s)} % TODO remove (s) or replace it by s?
\end{table}