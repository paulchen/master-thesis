...
public class CloudCover extends WeatherPhenomenon {
	...
	/**
	 * A constructor that creates an instance of <tt>CloudCover</tt> from a set
	 * of already existing instances of that class. The altitude will be set to
	 * the arithmetic mean of the altitudes of all these instances and the
	 * coverage will be set to the maximum of all values for the coverage field.
	 * 
	 * @param name
	 *            the unique name of the individual in the ontology that
	 *            corresponds to this object
	 * @param weatherPhenomena
	 *            a list of instances of <tt>CloudCover</tt> that should be used
	...
	/**
	 * A constructor that creates an instance of <tt>CloudCover</tt> given its
	 * unique name, its altitude and its coverage.
	 * 
	 * @param name
	 *            the unique name of the individual in the ontology that
	 *            corresponds to this object
	 ...
	 * @param coverage
	 *            the coverage of the cloud layer
	 */
	...
	public void createIndividuals(OntModel onto) {
		...
		onto.add(onto.createStatement(blankNode1,
				...
				onto.getResource(Weather.NAMESPACE + "okta")));
		...
		onto.add(onto.createStatement(blankNode2,
				onto.getProperty(Weather.MUO_NAMESPACE + "measuredIn"),
				onto.getResource(Weather.MUO_NAMESPACE + "meter")));
		...
		onto.add(onto.createStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasCloudCover"),
				blankNode1));
		onto.add(onto.createStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasCloudAltitude"),
				blankNode2));
	}
	...
	public TurtleStore getTurtleStatements() {
		...
		turtle.add(new TurtleStatement(blankNode1, Weather.MUO_PREFIX
				+ "numericalValue", String.valueOf(coverage)));
		turtle.add(new TurtleStatement(blankNode1, Weather.MUO_PREFIX
				+ "measuredIn", Weather.NAMESPACE_PREFIX + "okta"));

		turtle.add(new TurtleStatement(blankNode2, Weather.MUO_PREFIX
				+ "numericalValue", String.valueOf(altitude)));
		turtle.add(new TurtleStatement(blankNode2, Weather.MUO_PREFIX
				+ "measuredIn", Weather.MUO_PREFIX + "meter"));
		...
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasCloudCover", blankNode1));
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasCloudAltitude", blankNode2));
	...
	public void interpolate(WeatherPhenomenon intervalStartPhenomenon,
			...
			int current) {
		altitude = linearIntInterpolation(
				((CloudCover) intervalStartPhenomenon).getAltitude(),
				((CloudCover) intervalEndPhenomenon).getAltitude(), start, end,
				current);
		coverage = linearIntInterpolation(
				((CloudCover) intervalStartPhenomenon).getCoverage(),
				((CloudCover) intervalEndPhenomenon).getCoverage(), start, end,
				current);
	}
	...
	public WeatherPhenomenon createInterpolatedPhenomenon(String name,
		...
		cloudCover.interpolate(intervalStartPhenomenon, intervalEndPhenomenon,
				start, end, current);
		return cloudCover;
	}
