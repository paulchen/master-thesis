...
/**
 * This class represents a weather report for a certain period of time at a
 * certain location which has been obtained from a certain location.
 * 
 * @author Paul Staroch
 * 
 */
public class WeatherReport implements OntologyClass {
	/**
	 * Time when the data in this weather report has been obtained from the data
	 * source.
	 */
	...
	/**
	 * Instance of {@link WeatherState} which contains all weather data that
	 * belongs to this weather report.
	 */
	...
	/**
	 * Previous weather report obtained at the same time from the same source,
	 * if it exists.
	 */
	 ...
	 *            the date when the data represented by this report has been
	 *            obtained
	 * @param startTime
	 *            the start of the period of time the data in this weather
	 *            report is valid for
	 * @param endTime
	 *            the end of the period of time the data in this weather report
	 *            is valid for
	 * @param priority
	 *            the priority of all data in this weather report
	 * @param source
	 *            the source of all data of this weather report
	 * @param position
	 *            the geographical position this weather report is valid for
	 * @param weatherState
	 *            the instance of <tt>WeatherState</tt> that belongs to this
	 *            instance; it contains all weather data available for this
	 *            weather report
	 */
	public WeatherReport(String name, Instant observationTime,
			Interval startTime, Interval endTime, int priority,
			WeatherSource source, GeographicalPosition position,
	...
	public void createIndividuals(OntModel onto) {
		OntClass weatherReportClass = onto.getOntClass(Weather.NAMESPACE
				+ "WeatherReport");
		individual = onto.createIndividual(Weather.NAMESPACE + name,
				weatherReportClass);

		onto.add(onto.createLiteralStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasPriority"), priority));

		source.createIndividuals(onto);
		onto.add(onto.createStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasSource"),
				source.getIndividual()));

		startTime.createIndividuals(onto);
		endTime.createIndividuals(onto);
		onto.add(onto.createStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasStartTime"),
				startTime.getIndividual()));
		onto.add(onto.createStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasEndTime"),
				endTime.getIndividual()));

		position.createIndividuals(onto);
		onto.add(onto.createStatement(individual,
				onto.getProperty(GeographicalPosition.WGS84 + "location"),
				position.getIndividual()));

		observationTime.createIndividuals(onto);
		onto.add(onto.createLiteralStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasObservationTime"),
				observationTime.getIndividual()));
	...
	public TurtleStore getTurtleStatements() {
		...
		turtle.add(new TurtleStatement(getTurtleName(), "a",
				Weather.NAMESPACE_PREFIX + "WeatherReport"));
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasPriority", String
						.valueOf(priority)));

		turtle.addAll(source.getTurtleStatements());
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasSource", source.getTurtleName()));

		turtle.addAll(startTime.getTurtleStatements());
		turtle.addAll(endTime.getTurtleStatements());
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasStartTime", startTime
						.getTurtleName()));
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasEndTime", endTime
						.getTurtleName()));

		turtle.addAll(position.getTurtleStatements());
		turtle.add(new TurtleStatement(getTurtleName(),
				GeographicalPosition.WGS84_PREFIX + "location", position
						.getTurtleName()));

		turtle.addAll(observationTime.getTurtleStatements());
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasObservationTime",
				observationTime.getTurtleName()));
