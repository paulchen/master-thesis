...
public class Wind extends WeatherPhenomenon {
	...
	/**
	 * Wind direction (in degrees, North equals 0 degrees, East equals 90
	 * degrees etc.)
	 */
	private int windDirection;

	/**
	 * A constructor that creates an instance of <tt>Wind</tt> from a set of
	 * already existing instances of that class. Both wind speed and wind
	 * direction will be set to the arithmetic mean of the corresponding values
	 ...
	 *            a list of instances of <tt>Wind</tt> that should be used to
	...
	public void createIndividuals(OntModel onto) {
		...
		onto.add(onto.createStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasWindSpeed"),
				blankNode1));
		onto.add(onto.createStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasWindDirection"),
				blankNode2));
	}
	...
	public TurtleStore getTurtleStatements() {
		...
		turtle.add(new TurtleStatement(blankNode1, Weather.MUO_PREFIX
				+ "numericalValue", "\""
				+ String.valueOf(roundFloat(windSpeed, Weather.DECIMALS))
				+ "\"^^xsd:float"));
		turtle.add(new TurtleStatement(blankNode1, Weather.MUO_PREFIX
				+ "measuredIn", Weather.NAMESPACE_PREFIX + "metresPerSecond"));

		turtle.add(new TurtleStatement(blankNode2, Weather.MUO_PREFIX
				+ "numericalValue", String.valueOf(windDirection)));
		turtle.add(new TurtleStatement(blankNode2, Weather.MUO_PREFIX
		...
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasWindSpeed", blankNode1));
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasWindDirection", blankNode2));
	...
	public void interpolate(WeatherPhenomenon intervalStartPhenomenon,
			...
			int current) {
		windSpeed = linearFloatInterpolation(
				((Wind) intervalStartPhenomenon).getWindSpeed(),
				((Wind) intervalEndPhenomenon).getWindSpeed(), start, end,
				current);
		windDirection = linearIntInterpolation(
				((Wind) intervalStartPhenomenon).getWindDirection(),
				((Wind) intervalEndPhenomenon).getWindDirection(), start, end,
				current);
	}
	...
	public WeatherPhenomenon createInterpolatedPhenomenon(String name,
		...
		wind.interpolate(intervalStartPhenomenon, intervalEndPhenomenon, start,
				end, current);
		return wind;
	}
