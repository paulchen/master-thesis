...
public class Temperature extends WeatherPhenomenon {
	...
	/**
	 * A constructor that creates an instance of <tt>Temperature</tt> from a set
	 * of already existing instances of that class. The temperature value will
	 * be set to the arithmetic mean of the temperature values of all these
	 * instances.
	 * 
	 * @param name
	 ...
	 *            a list of instances of <tt>Temperature</tt> that should be
	...
	public Temperature(String name, List<WeatherPhenomenon> weatherPhenomena) {
		...
		for (WeatherPhenomenon phenomenon : weatherPhenomena) {
			temperatureValue += ((Temperature) phenomenon)
					.getTemperatureValue();
		}
	...
	/**
	 * A constructor that creates an instance of <tt>Temperature</tt> given its
	 * unique name and its temperature value.
	 * 
	 * @param name
	 *            the unique name of the individual in the ontology that
	 *            corresponds to this object
	...
	public void createIndividuals(OntModel onto) {
		...
		onto.add(onto.createLiteralStatement(blankNode,
				...
				roundFloat(temperatureValue, Weather.DECIMALS)));
		...
		onto.add(onto.createStatement(individual,
				onto.getProperty(Weather.NAMESPACE + "hasTemperatureValue"),
				blankNode));
	}
	...
	public TurtleStore getTurtleStatements() {
		...
		turtle.add(new TurtleStatement(blankNode, Weather.MUO_PREFIX
				+ "numericalValue",
				"\""
						+ String.valueOf(roundFloat(temperatureValue,
		...
		turtle.add(new TurtleStatement(getTurtleName(),
				Weather.NAMESPACE_PREFIX + "hasTemperatureValue", blankNode));
	...
	public void interpolate(WeatherPhenomenon intervalStartPhenomenon,
			...
			int current) {
		temperatureValue = linearFloatInterpolation(
				((Temperature) intervalStartPhenomenon).getTemperatureValue(),
				((Temperature) intervalEndPhenomenon).getTemperatureValue(),
				start, end, current);
	}
	...
	public WeatherPhenomenon createInterpolatedPhenomenon(String name,
		...
		temperature.interpolate(intervalStartPhenomenon, intervalEndPhenomenon,
				start, end, current);
		return temperature;
	}
